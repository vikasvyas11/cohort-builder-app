...
from modules.metrics_engine import (
    compute_intra_metrics, compute_inter_metrics,
    compute_confusion_matrix, compute_truth_space, compute_crl_score,
)
...
def page_analysis():
    ...
    if st.button(run_btn_label, type="primary"):
        with st.spinner(
            ...
            try:
                ...
                # Compute confusion matrix using cluster column as ground truth
                cm = compute_confusion_matrix(
                    results["df_predict"],
                    st.session_state["fakea"],
                    st.session_state["fakeb"],
                    st.session_state["operation_mode"],
                )
                st.session_state["run1_cm"] = cm
                # Compute truth space (precision/recall curve) for probabilistic mode
                if st.session_state["linkage_type"] == "probabilistic":
                    ts = compute_truth_space(
                        results["df_predict"],
                        st.session_state["fakea"],
                        st.session_state["fakeb"],
                        st.session_state["operation_mode"],
                    ...
                    st.session_state["run1_crl"] = compute_crl_score(ts)
                else:
                    st.session_state["run1_ts"] = None
                    st.session_state["run1_crl"] = {}
    ...
    tab_edges, tab_clusters, tab_demo, tab_studio, tab_cm, tab_data = st.tabs([
        "Edge Metrics",
        "Cluster Metrics",
        "Demographics",
        "Cluster Studio",
    ...
    with tab_cm:
        ...
        st.write(
            "Ground truth: the 'cluster' column in the original dataset. "
            "Records with the same cluster value are true matches. "
            "This comparison evaluates how accurately the linkage model identified "
            "the true entity pairs."
        )
        ...
        if not cm or "error" in cm:
            st.info(cm.get("error", "Confusion matrix not yet computed. Run analysis first."))
        else:
            # KPI row
            ...
            with mc1:
                ...
                metrics_df = pd.DataFrame([
                    ...
                    {"Metric": "F1 Score",   "Value": f"{cm.get('f1', 0):.4f}",
                     "Meaning": "Harmonic mean of P and R"},
        ...
        # Precision-Recall curve (probabilistic mode only)
        if ts is not None and not ts.empty:
            st.divider()
            st.subheader("Precision-Recall Curve and F* Score (example 16 - CRL analysis)")
            st.write(
                "These curves show Precision (1-FDR) and Recall (1-FNR) at every "
                "match-probability threshold. The F* curve shows TP/(TP+FP+FN). "
                "Use these to select an operating threshold that balances precision "
                "and recall for your use case."
            )
            pr_col, fs_col = st.columns(2)

            ts_valid = ts.dropna(subset=["precision_val", "recall_val"])
            if not ts_valid.empty:
                with pr_col:
                    fig_pr = px.line(
                        ts_valid, x="recall_val", y="precision_val",
                        title="Precision-Recall Curve",
                        labels={"recall_val": "Recall (1-FNR)",
                                "precision_val": "Precision (1-FDR)"},
                        template="simple_white",
                        color_discrete_sequence=["#1E6EC4"],
                    )
                    fig_pr.update_layout(height=320, xaxis_range=[0, 1], yaxis_range=[0, 1.05])
            ...
            if not ts_fs.empty:
                with fs_col:
                    fig_fs = px.line(
                        ts_fs, x="match_probability", y="fstar",
                        title="F* Score vs Threshold",
                        labels={"match_probability": "Match Probability Threshold",
                                "fstar": "F* Score"},
                        template="simple_white",
                        color_discrete_sequence=["#28A060"],
                    )
                    fig_fs.update_layout(height=320, xaxis_range=[0, 1], yaxis_range=[0, 1.05])
                    st.plotly_chart(fig_fs, use_container_width=True)

            # CRL score metric
            if crl.get("crl_score") is not None:
                st.divider()
                st.subheader("CRL Score (Composite Reliability of Linkage)")
                st.write(
                    "The CRL score measures linkage quality in the valid operating region "
                    "where both FDR and FNR are within epsilon. "
                    "CRL = AVG(F*) * (t_upper - t_lower). Higher = more reliable linkage."
                )
                cr1, cr2, cr3, cr4 = st.columns(4)
                cr1.metric("CRL Score",  f"{crl.get('crl_score', 0):.6f}")
                cr2.metric("t_upper",    f"{crl.get('t_upper', 'N/A')}")
                cr3.metric("t_lower",    f"{crl.get('t_lower', 'N/A')}")
                cr4.metric("epsilon_z",  f"{crl.get('epsilon_z', 'N/A')}")
    ...
    if st.button("Generate PDF report for Run 1"):
        with st.spinner("Generating SeRP-style PDF report..."):
            try:
                pdf_bytes = generate_report(
                    ...
                    # Explicit None check avoids pandas "ambiguous truth value" error
                    # that occurs when using `or` with a non-empty DataFrame
                    truth_space_df=(
